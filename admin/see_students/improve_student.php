...
<body>
    ...
    <div style="display:flex;">
        <div>
            <h2 style="color:#ff0000;font-family:'Acme';" class = "page-header mr-5"><u>Students Registered For Improvement</u></h2>
        </div>
    ...
    <?php
        ...
        if($value){
            $query1 = "SELECT * FROM registration JOIN sem_reg ON registration.username = sem_reg.username WHERE role='Improve' and continue_batch='{$value}'";
    
            $result = mysqli_query($conn,$query1);
            $num_rows = mysqli_num_rows($result);
            if(!$num_rows){
                echo "<h2 style='color:red;color:#ff0000;font-family:Acme;position:relative;top:35px;left:2%;''>No Students Have Registered For Improvement.</h2>";
            }else{
    ?>
    <div class = "col-lg-12 mcon text-center">
        ...
        <?php 
            ...
            $query1 = "SELECT * FROM registration JOIN sem_reg ON registration.username = sem_reg.username WHERE role='Improve' order by continue_batch";
            $result = mysqli_query($conn,$query1);
            $num_rows = mysqli_num_rows($result);
            $result = mysqli_query($conn,$query1);
            $num_rows = mysqli_num_rows($result);
            if(!$num_rows){
                echo "<h2 style='color:red;color:#ff0000;font-family:Acme;position:relative;top:35px;left:2%;''>No Students Have Registered For Improvement Exam Yet.</h2>";
            }else{ ?>
                <table id="mytab1" width = "100%" class="mtab table table-bordered table-hover table-responsive" style = "margin-top:0;">
    	    <tr align = 'center' class = "text-center" style = "background-color:rgba(0,128,128,0.3);color:black;font-size:25px;text-align:center;font-family: 'PT serif';" class="table-info">
                <th style='border:2px solid black'>SL.No</th>
