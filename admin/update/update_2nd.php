...
<?php
    if(isset($_POST['Update'])){
       ...
       $final_update = "UPDATE add_s2 set sub_name = '{$sub_name}',sub_code = '{$sub_code}',credit = '{$credit}' where id = {$update_id}";

       $update_run = mysqli_query($conn,$final_update);
       if ($update_run) {
            $err = "<font color='green' align='center'>Subject Updated Successfully...!</font>";
            header('location:http://localhost/exam_form/admin/dashboard.php?page=disp_s2');
        } else {
...
<body>
    ...
    <div class="wrapper">
        ...
        <div class="forms" style="width: 60%;">
            <?php
                $update_id = $_GET['id'];
                $fetch_select = "SELECT * FROM add_s2 WHERE id='$update_id'";
